...
export class UsersService {
    ...
    try {
      ...
      if (newUser.type !== userTypes.ADMIN) {
        sendEmail(
          ...
          'Email verification - Compra Facil',
          {
      ...
      return {
        ...
        message:
          newUser.type === userTypes.ADMIN
            ...
            : 'Por favor activa tu cuenta. Te enviamos un correo con tu OTP',
  ...
  async login(email: string, password: string) {
    try {
      ...
      if (!userExists.isVerified) {
        throw new Error('Verifica tu email');
      }
  ...
  async confirmedEmail(otp: string, email: string) {
    try {
      ...
      return {
        success: true,
        message: 'Email verificado correctamente, ya puedes logearte ',
  ...
  async sendOtpEmail(email) {
    try {
      ...
      if (!emailverified) throw new Error('Email no encontrado');

      if (emailverified.isVerified) throw new Error('Email verificado');
      ...
      sendEmail(
        emailverified.email,
        envs.verifyEmail,
        'Email verificacion - SOA-Ecommerce',
        {
      ...
      return { success: true, message: 'OTP enviado correctamente', result: { email: emailverified.email } };
  ...
  async forgotPassword(email:string){
    try {
      ...
      sendEmail(
        user.email,
        envs.forgotPassword,
        'Password reseteado - SOA-Ecommerce',
        {
